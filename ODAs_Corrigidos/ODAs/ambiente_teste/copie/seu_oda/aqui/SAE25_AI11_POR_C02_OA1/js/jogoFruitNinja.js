var AppFruit = function (
  ...
  _temVideo = false,
  _temSprite = false,
  mensagensFinais = null,
  mensagensFinaisSons = null,
  _musica = null
) {
  var canvas,
    pincel,
    ...
    if (iniciar) {
      if (jogaFrutas) {
        ...
        if (rate > frequencia) {
          ...
          if (_temSprite) {
            var bob1 = new createjs.Sprite(
              _itens[fase].figuras[qual],
              "fumaca1"
            );
            bob1.scaleX = bob1.scaleY = 0.1;
            createjs.Tween.get(bob1).to(
              { scaleX: 1, scaleY: 1 },
              250,
            ...
            grupo[i] = new createjs.Bitmap(
              caminho + _itens[fase].figuras[qual]
            );
  ...
  function verificaFim() {
    ...
    if (mensagensFinais) {
      if (i_acertos >= 0 && i_acertos <= 15) {
        txt = new createjs.Text(
          mensagensFinais[0],
          tamanhoTexto + "px VAG Rounded BT",
          "#ffffff"
        );
    ...
    if (continua) {
      ...
      createjs.Tween.get(bo)
        .wait(1000)
        .to({ y: 350 }, 1000, createjs.Ease.backOut);
  ...
  function iniciaFase(conta) {
    ...
    if (fase < _itens.length) {
      ...
      texto_tempo = new createjs.Text(
        "00s",
        "bold 80px VAG Rounded BT",
        "#000000"
      );
      texto_tempo.x = 1150;
      texto_tempo.y = 30;
  ...
  function animaTitulo(texto) {
    ...
    if (extensao == "jpg" || extensao == "png") {
      ...
      txt.lineWidth = 900;
      txt.textAlign = "center";
      var contorno = new createjs.Text(
        texto,
        tamanhoTexto + "px VAG Rounded BT",
        "#000000"
      );
