(function(window)
{
	...
	/* PROPIERTIES */
		...
		var sendingProtocol = checker.iphone || checker.firefox ? 'about:':'tuoba:';		
	...
	/* METHODS */	
		...
		{
			var _jsonData = b64_utf8(jsonArgs);
			var _serializeObject = JSON.parse( _jsonData );
		...
		var call = function( )
		{
			...
			setTimeout( function(){ window.location.href=sendingProtocol+'[SWVData]'+utf8_b64( JSON.stringify( _serializeObject ) );},aggregatedCallDelay );			
		...
		var onGetFilePaths = function( data )
		{
			...
			pathsReady = true;
			for (var i = 0; i < onDeviceReadyHandlers.length; i++ )
				onDeviceReadyHandlers[i]();
...
function utf8_b64(str) { return window.btoa(unescape(encodeURIComponent(str))); }
function b64_utf8(str) { return decodeURIComponent(escape(window.atob(str))); }